...
def run_topology_demo():  # type: ignore[no-untyped-def]
    ...
    trajectories = [
        ...
        LatentTrajectory(agent_id="agent_4_hallucinator", hidden_states=t4_hallucinator, confidence=1.0),
    ]
    ...
    rejected_agents = [t.agent_id for t in trajectories if t.metadata.get("rejected", False)]


    assert "agent_4_hallucinator" in rejected_agents, "ERROR: Filter failed to catch hallucinator."

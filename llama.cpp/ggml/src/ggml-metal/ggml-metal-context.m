...
struct ggml_metal {
  ...
  // the last command buffer queued into the Metal queue with operations
  // relevant to the current Metal backend
...
ggml_metal_t ggml_metal_init(ggml_metal_device_t dev) {
  ...
  // TODO: would it be better to have one queue for the backend and one queue
  // for the device?
  //       the graph encoders and async ops would use the backend queue while
  //       the sync ops would use the device queue?
  ...
  if (res->lib == NULL) {
    GGML_LOG_WARN("%s: the device does not have a precompiled Metal library - "
                  "this is unexpected\n",
                  __func__);
  ...
  const struct ggml_metal_device_props *props_dev =
      ggml_metal_device_get_props(dev);
  ...
  GGML_LOG_INFO("%s: use fusion         = %s\n", __func__,
                res->use_fusion ? "true" : "false");
  GGML_LOG_INFO("%s: use concurrency    = %s\n", __func__,
                res->use_concurrency ? "true" : "false");
  GGML_LOG_INFO("%s: use graph optimize = %s\n", __func__,
                res->use_graph_optimize ? "true" : "false");
  ...
  if (ctx->debug_fusion > 0) {
    ...
    for (int i = 0; i < GGML_OP_COUNT; i++) {
      ...
      GGML_LOG_DEBUG("%s: - %s: %" PRIu64 "\n", __func__,
                     ggml_op_name((enum ggml_op)i), ctx->fuse_cnt[i]);
    }
  }
...
const char *ggml_metal_get_name(ggml_metal_t ctx) { return ctx->name; }
...
static struct ggml_metal_buffer_id
ggml_metal_get_buffer_id(const struct ggml_tensor *t) {
  if (!t) {
...
void ggml_metal_set_tensor_async(ggml_metal_t ctx, struct ggml_tensor *tensor,
  ...
  @autoreleasepool {
    ...
    // this will be queued at the end, after any currently ongoing GPU
    // operations
    ...
    // instead, remember a reference to the command buffer and wait for it later
    // if needed
...
void ggml_metal_get_tensor_async(ggml_metal_t ctx,
...
bool ggml_metal_cpy_tensor_async(ggml_metal_t ctx_src, ggml_metal_t ctx_dst,
...
enum ggml_status ggml_metal_graph_compute(ggml_metal_t ctx,
                                          struct ggml_cgraph *gf) {
  ...
  // submit the ggml compute graph to the GPU by creating command buffers and
  // encoding the ops in them the first n_nodes_0 are encoded and submitted for
  // processing directly by the calling thread while these nodes are processing,
  // we start n_cb threads to enqueue the rest of the nodes each thread creates
  // it's own command buffer and enqueues the ops in parallel
  //
  // tests on M1 Pro and M2 Ultra using LLaMA models, show that optimal values
  // for n_cb are 1 or 2
  ...
  @autoreleasepool {
    ...
    if (use_capture) {
      ...
      // make sure all previous computations have finished before starting the
      // capture
      if (ctx->cmd_buf_last) {
        [ctx->cmd_buf_last waitUntilCompleted];
        ctx->cmd_buf_last = nil;
      ...
      if (!ctx->capture_started) {
        ...
        id<MTLDevice> device = ggml_metal_device_get_obj(ctx->dev);
        ctx->capture_scope = [[MTLCaptureManager sharedCaptureManager]
        ...
        descriptor.outputURL = [NSURL
            fileURLWithPath:[NSString
                                stringWithFormat:@"/tmp/perf-metal.gputrace"]];
    ...
    // enter here only when capturing in order to wait for all computation to
    // finish otherwise, we leave the graph to compute asynchronously
    if (!use_capture && ctx->capture_started) {
      ...
      for (int i = 0; i < n_cb; ++i) {
        ...
        id<MTLCommandBuffer> next_buffer =
            (i + 1 < n_cb ? ctx->cmd_bufs[i + 1].obj : nil);
        if (!next_buffer) {
          continue;
        }

        const bool next_queued =
            ([next_buffer status] != MTLCommandBufferStatusNotEnqueued);
        if (next_queued) {
          continue;
        }
...
void ggml_metal_set_n_cb(ggml_metal_t ctx, int n_cb) {
  if (ctx->n_cb != n_cb) {
    ...
    if (ctx->n_cb > 2) {
      GGML_LOG_WARN("%s: n_cb = %d, using n_cb > 2 is not recommended and can "
                    "degrade the performance in some cases\n",
                    __func__, n_cb);
    }
  }
  ...
  ctx->encode_async = Block_copy(^(size_t iter) {
    ...
    if (cb_idx < n_cb_l) {
      ...
      idx_end = n_nodes_0 +
                (MIN((cb_idx == n_cb_l - 1) ? n_nodes_1
                                            : (cb_idx + 1) * n_nodes_per_cb,
                     n_nodes_1));
    }
    ...
    ggml_metal_op_t ctx_op = ggml_metal_op_init(
        ctx->dev, cmd_buf, ctx->gf, idx_start, idx_end, ctx->use_fusion,
        ctx->use_concurrency, ctx->capture_next_compute, ctx->debug_graph,
        ctx->debug_fusion);
...
void ggml_metal_set_abort_callback(ggml_metal_t ctx,
                                   ggml_abort_callback abort_callback,
                                   void *user_data) {
...
void ggml_metal_preload_dora(ggml_metal_t ctx, void *data, size_t size) {
  if (ctx) {
    if (!ctx->preload_dora_buffer) {
      id<MTLDevice> device = ggml_metal_device_get_obj(ctx->dev);
      ctx->preload_dora_buffer =
          [device newBufferWithLength:size options:MTLResourceStorageModeShared]
              .contents;
    }
    memcpy(ctx->preload_dora_buffer, data, size);
    ctx->preload_ready = true;
    GGML_LOG_DEBUG("%s: DoRA weights preloaded into TT-Distill buffer\n",
                   __func__);
  }
}

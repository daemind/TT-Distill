...
class LlamaRawEmbed:
    ...
    def embed_tensor(self, embedding_tensor: np.ndarray) -> np.ndarray:
        ...
        raw_batch = getattr(batch_wrapper, "batch", None) or getattr(batch_wrapper, "_batch", None)

        if raw_batch is None:
            raise RuntimeError("Impossible de trouver la structure C llama_batch interne.")
        ...
        raw_batch.token = ctypes.cast(
            ctypes.c_void_p(0),
            ctypes.POINTER(llama_cpp_module.llama_token)
        )
        ...
        if not embeddings_ptr:
            raise RuntimeError("Le pointeur d'embedding est NULL. Le contexte est-il en mode embedding=True ?")
        ...
        # 6. Nettoyage de sécurité : on débranche le pointeur
        raw_batch.embd = ctypes.cast(
            raw_batch.embd,
            ctypes.POINTER(ctypes.c_float)
        )

...
class MACASalonBridge:
    ...
    def __init__(
        ...
        self.maca_engine = maca_engine or MACAEngine(latent_dim=latent_dim, seq_len=seq_len)
        ...
        logger.info(f"🌉 MACASalonBridge initialisé (latent_dim={latent_dim}, seq_len={seq_len})")
    ...
    async def create_hybrid_session(
        ...
        logger.info(f"✅ Session hybride créée: {title} (id={hybrid_session.session_id})")
    ...
    async def add_text_message(
        ...
        if session_id in self._hybrid_sessions:
            ...
            logger.debug(f"📝 Message textuel converti en trajectoire latente: {agent_name}")
    ...
    async def _text_to_latent(self, text: str, agent_name: str) -> np.ndarray:
        ...
        latent_state += np.random.randn(self.maca_engine.latent_dim) * 0.1 * (agent_hash % 100) / 100
    ...
    async def run_tensorial_consensus(
        ...
        hybrid_session.status = "resolved" if consensus.metadata["is_valid"] else "pending"

...
# Constants
SINKHORN_ITERATIONS = 20  # Nombre d'itérations pour l'algorithme de Sinkhorn (optimisé: 50→20)
...
SINKHORN_CONVERGENCE_THRESHOLD = 1e-6  # Seuil de convergence pour l'algorithme de Sinkhorn
...
class SinkhornBarycenter:
    ...
    def compute_barycenter(
        ...
        aligned_array = np.stack(aligned_states, axis=0)  # Shape: (n_agents, max_seq_len, latent_dim)
        ...
        if len(trajectories) > 2 and rejection_threshold > 0.0:
            ...
            for i, traj_array in enumerate(aligned_array):
                ...
                cosine_sim = float(np.dot(flat_traj, flat_centroid) / (norm_traj * norm_centroid))
        ...
        barycentre = np.average(aligned_array, weights=weights, axis=0)  # Shape: (max_seq_len, latent_dim)
        ...
        if participating_indices:
            ...
            variance_list = [np.var(t.hidden_states - barycentre[: t.seq_len]) for t in valid_trajectories]
            variance = float(np.mean(variance_list)) if variance_list else 1.0
        else:
            variance = float(np.mean([np.var(t.hidden_states - barycentre[: t.seq_len]) for t in trajectories]))
...
class MACAEngine:
    ...
    def __init__(
        ...
        logger.info(f"🧠 MACA Engine initialisé (latent_dim={latent_dim}, seq_len={seq_len})")
    ...
    async def run_consensus(
        ...
        Returns:
            ConsensusResult avec le barycentre et les métriques
        """
        logger.info(f"🎮 Démarrage du consensus MACA avec {len(agent_intentions)} agents")

        # Étape 1: Latent Rollout pour chaque agent
        trajectories: list[LatentTrajectory] = []
        for intent in agent_intentions:
            agent_id = intent["agent_id"]
            initial_state = intent.get("initial_state", np.random.randn(self.latent_dim))
            ...
            logger.debug(f"✅ Agent {agent_id}: trajectoire générée (len={trajectory.seq_len})")
...
class TTDistillBridge:
    ...
    def distill_barycenter(
        ...
        logger.info(f"🔥 Distillation du barycentre vers DoRA (arch={arch}, SVD={use_svd})")
        ...
        if use_svd:
            ...
            lora_a = (u[:, :effective_rank].T * np.sqrt(s[:effective_rank])).astype(np.float32)
            lora_b = vt[:effective_rank, :].T.astype(np.float32)

            logger.info(f"✅ SVD factorisation: rank={effective_rank}, singular_values={s[:effective_rank]}")
            logger.info(f"   lora_a shape: {lora_a.shape}, lora_b shape: {lora_b.shape}")
        else:
            ...
            lora_a = np.random.randn(self.rank, effective_hidden_size).astype(np.float32)
            # Appliquer la direction du barycentre
            lora_a *= np.outer(np.random.randn(self.rank), barycentre_norm[:effective_hidden_size])
        ...
        scale = (self.adapter_size_mb * 1024 * 1024 / (lora_a.nbytes + lora_b.nbytes)) ** 0.5
        lora_a *= scale
        ...
        logger.info(f"   dora_scale shape: {dora_scale.shape}, magnitude={magnitude:.6f}")
    ...
    def save_adapter(
        ...
        logger.info(f"✅ Adaptateur sauvegardé: {output_path} ({actual_size_mb:.2f} MB)")

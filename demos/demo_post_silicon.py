    ...
    def get_average_metrics(self) -> LatencyMetrics:
        ...
        return LatencyMetrics(
            ...
            distillation_time_ms=float(
                np.mean([m.distillation_time_ms for m in self.history])
            ),
            ...
            reflex_frequency_hz=float(
                np.mean([m.reflex_frequency_hz for m in self.history])
            ),
        )
...
def print_profiling_summary(
    ...
    # Métriques matérielles
    hw_avg = (
        hardware_profiler.history[-1]
        if hardware_profiler.history
        else HardwareMetrics()
    )
    ...
    table.add_row(
        "MACA",
        f"{pipeline_avg.maca_time_ms:.2f} ms",
        f"{(pipeline_avg.maca_time_ms / total * 100):.1f}%",
    )
    table.add_row(
        "DoRA Distillation",
        f"{pipeline_avg.distillation_time_ms:.2f} ms",
        f"{(pipeline_avg.distillation_time_ms / total * 100):.1f}%",
    )
    table.add_row(
        "Reflex S1",
        f"{pipeline_avg.reflex_time_ms:.2f} ms",
        f"{(pipeline_avg.reflex_time_ms / total * 100):.1f}%",
    )
    table.add_row(
        "Vector Memory RAG",
        f"{pipeline_avg.rag_time_ms:.2f} ms",
        f"{(pipeline_avg.rag_time_ms / total * 100):.1f}%",
    )
    ...
    console.print(
        f"[dim]Répartition latence:[/dim] MACA={pipeline_avg.maca_time_ms:.2f}ms, DoRA={pipeline_avg.distillation_time_ms:.2f}ms, Reflex={pipeline_avg.reflex_time_ms:.2f}ms"
    )
    console.print(
        f"[dim]Fréquence réflexe:[/dim] {pipeline_avg.reflex_frequency_hz:.1f} Hz"
    )
...
async def _main() -> None:
    ...
    console.print("[dim]Hardware Profiling - Harmonie Mac Studio (M2 Max/ANE)[/dim]")
    console.print("=" * 70)

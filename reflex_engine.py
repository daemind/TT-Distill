...
class ReflexEngine:
    ...
    def query_with_embeds(self, embeddings: np.ndarray, prompt: str = STABILITY_PROMPT) -> tuple[float, str]:
        """
    ...
    try:
        for event in node:
            if event["type"] == "INPUT" and event["id"] == "intent_tensor":
                ...
                latency_ms, _response_text = reflex_engine.query_reflex(STABILITY_PROMPT)

                # 2. Envoyer la commande moteur (latence mesurée)
                node.send_output("motor_command", np.array([latency_ms], dtype=np.float32).tobytes())

                # 3. Afficher le statut

    finally:
        # Libérer les ressources
        reflex_engine.close()  # type: ignore[no-untyped-call]
...
class MultimodalReflexEngine(ReflexEngine):
    ...
    def query_with_video(self, prompt: str, video_path: str, frame_indices: list[int] | None = None) -> tuple[float, str]:
        """
        ...
        # Extraire les frames de la vidéo (simplifié - utiliser opencv en production)
        # Pour l'instant, on utilise une approche basique
        import base64
        import subprocess
        ...
        for idx in frame_indices:
            ...
            result = subprocess.run(
                ["ffmpeg", "-i", video_path, "-ss", str(idx), "-vframes", "1", "-f", "image2", "-"],
                capture_output=True
            )
            if result.stdout:

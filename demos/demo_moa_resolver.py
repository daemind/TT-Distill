...
ARC_COLORS = {
    ...
    5: "grey66", 6: "magenta", 7: "orange3", 8: "cyan", 9: "dark_red",
}
...
def render_grid(grid: list[list[int]], title: str) -> Panel:
    ...
    for row in grid:
        ...
        display += "\n"
    return Panel(Text.from_markup(display.rstrip()), title=title, border_style="dim", expand=False)
...
async def run_moa_demo() -> None:
    ...
    console.print("\n" + "=" * 80)
    console.print("[bold cyan]🔬 DÉMO 6: RÉSOLVEUR UNIVERSEL ARC (MoA + Heuristiques + Latent Math)[/bold cyan]")
    console.print("=" * 80)
    console.print(
    ...
    metal_status = "[green]ACTIVÉ[/green]" if gater.metal_available else "[yellow]numpy-only[/yellow]"
    ...
    console.print(f"📋 Chargé [bold]{len(tasks)}[/bold] tâches depuis [dim]{arc_dir}[/dim]\n")
    ...
    console.print(f"\n{'=' * 80}")
    console.print(f"[bold green]🏆 RÉSULTATS HEURISTIQUE: {n_solved}/{n_total} ({solve_rate:.1f}%)[/bold green]")
    console.print(f"[bold magenta]🧠 RÉSULTATS MATH PURE: {n_math_solved}/{n_total} ({math_solve_rate:.1f}%)[/bold magenta]")
    console.print(f"[bold yellow]⚡ RÉSULTATS LATENT MATH: {n_hybrid_solved}/{n_total} ({hybrid_solve_rate:.1f}%)[/bold yellow]")
    console.print(f"{'=' * 80}\n")

    # Strategy breakdown
    strat_table = Table(title="📊 Stratégies Utilisées (Heuristique)", show_header=True, header_style="bold magenta")
    ...
    perf_table.add_row("Tâches résolues", f"[bold green]{n_solved}[/bold green]", f"[bold magenta]{n_math_solved}[/bold magenta]", f"[bold yellow]{n_hybrid_solved}[/bold yellow]")
    perf_table.add_row("Solve rate", f"[bold green]{solve_rate:.1f}%[/bold green]", f"[bold magenta]{math_solve_rate:.1f}%[/bold magenta]", f"[bold yellow]{hybrid_solve_rate:.1f}%[/bold yellow]")
    perf_table.add_row("Moy. solve/tâche", f"{avg_solve:.3f} ms", f"{avg_math_solve:.3f} ms", f"{avg_hybrid_solve:.3f} ms")
    perf_table.add_row("Total solve time", f"{total_solve_ms:.1f} ms", f"{total_math_solve_ms:.1f} ms", f"{total_hybrid_solve_ms:.1f} ms")
    ...
    console.print(
        Panel(
            ...
            + ", ".join(f"{s} ({c})" for s, c in sorted(strategy_counts.items(), key=lambda x: -x[1])[:3]),
            title="📊 Score Final",
            border_style="green",
        )
    )

...
class MoAGater:
    ...
    compiled with TT-Distill patches), the gater pulls fused adapters into
    ...
    def metal_available(self) -> bool:
        """Whether the Metal O(1) swap backend is loaded (always True if initialized)."""
        return True
        ...
        for i, adapter in enumerate(loaded_adapters[1:], start=1):
            ...
            for key in base_keys:
                if base_adapter[key].shape != adapter[key].shape:
                    raise ValueError(
                        f"Shape mismatch for key '{key}' between adapter 0 and adapter {i}"
                    )
    ...
    def preload_to_metal(self, adapter: dict[str, np.ndarray]) -> float:
        ...
        This uses the hardware-native ``ggml_metal_preload_dora`` CTypes call
        to project the data directly into the Metal context, enabling sub-0.1ms
        swapping.
        ...
        # Flatten only numpy tensors into one contiguous buffer (C-order, float32)
        arrays = [
        ...
        if not arrays:
            logger.warning("MoAGater: no tensors found to preload")
            return 0.0
        ...
        self._last_preloaded_buffer = ptr
        # Keep a reference to prevent GC
        self._preload_data = contiguous

        # Execute hardware-native mmap/preload
        self._swapper.preload(ptr.value if ptr.value else 0, contiguous.nbytes)
    ...
    def swap_active_adapter(self) -> float:
        ...
        """
        # Call the robust public swap function for the O(1) pointer exchange
        return self._swapper.swap(0)  # Passing NULL as backend_ptr for context-global swap

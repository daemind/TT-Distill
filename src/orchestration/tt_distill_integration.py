...
from src.orchestration.maca import ConsensusResult, MACAEngine, TTDistillBridge
...
class TTDistillIntegration:
    ...
    def __init__(self, config: TTDistillConfig | None = None):
        ...
        # MoA Gater with Metal O(1) swap (STRICT REQUIREMENT)
        ...
        logger.info("✅ TT-Distill Integration initialisée (STRICT Hardware Mode)")
    ...
    async def initialize(self) -> None:
        ...
        self._optimization_task = asyncio.create_task(
            self.post_silicon.run_optimization_loop(
                self.config.post_silicon_interval_ms
            )
        )
    ...
    async def run_consensus(self, intents: list[str]) -> ConsensusResult:
        ...
        logger.debug(f"✅ Consensus atteint en {latency_ms:.2f} ms")
    ...
    def distill_to_dora(self, consensus: ConsensusResult) -> np.ndarray:
        ...
        """
        # Utiliser le bridge rigoureux pour la distillation SVD
        bridge = TTDistillBridge(
            rank=self.config.maca_rank, hidden_size=self.config.vector_embedding_dim
        )

        # Distiller (avec préservation temporelle si le barycentre est 2D)
        # On force preserve_temporal=False ici pour obtenir un adaptateur de dimension [1, dim]
        # compatible avec l'exécution réflexe actuelle qui attend un vecteur de poids.
        adapter_dict = bridge.distill_barycenter(
            consensus.barycentre, use_svd=True, preserve_temporal=False
        )

        lora_a = adapter_dict["lora_a"]
        lora_b = adapter_dict["lora_b"]

        # Combiner en un seul adaptateur pour le retour
        # A est (rank, seq), B est (dim, rank)
        # On veut (seq, dim) -> A.T @ B.T
        dora_adapter = lora_a.T @ lora_b.T

        logger.debug(
            f"✅ DoRA adapter généré via SVD: shape={dora_adapter.shape}, rank={self.config.maca_rank}"
        )
        ...
        logger.debug(
            f"⚡ DoRA adapter staged for Metal O(1) swap (preload: {preload_ms:.4f} ms)"
        )

        return cast(np.ndarray, dora_adapter)
    ...
    async def execute_reflex(
        ...
        if self._pending_swap:
            ...
            logger.debug(
                f"⚡ Metal O(1) swap fired before inference ({swap_ms:.6f} ms)"
            )
    ...
    async def execute_multimodal_reflex(
        ...
        if image_path and Path(image_path).exists():  # noqa: ASYNC240
            latency_ms, response_text = self.reflex_engine.query_with_image(
                prompt, image_path
            )
        elif video_path and Path(video_path).exists():  # noqa: ASYNC240
            latency_ms, response_text = self.reflex_engine.query_with_video(
                prompt, video_path
            )
        else:
            latency_ms, response_text = self.reflex_engine.query_reflex(prompt)
    ...
    def _generate_latent_trajectory(self, intent: str) -> np.ndarray:
        """Générer un état latent initial pour un agent S2 via VectorMemory.

        Args:
            intent: Intent de l'agent (texte brut)

        Returns:
            np.ndarray: État latent initial (shape: [hidden_size])
        """
        hidden_size = self.config.vector_embedding_dim

        # EXÉCUTION RÉELLE: Utiliser les embeddings GGUF/NPU (Zero-Fake)
        embedding = self.vector_memory.get_embedding(intent)

        if embedding is None:
            msg = f"VectorMemory could not generate embedding for intent: {intent}. Hardware vector bridge failure."
            logger.error(f"❌ {msg}")
            raise RuntimeError(msg)

        # S'assurer que la dimension correspond (padding/truncation si nécessaire)
        vec = np.array(embedding, dtype=np.float32)
        if len(vec) == hidden_size:
            return vec
        if len(vec) > hidden_size:
            return vec[:hidden_size]
        # Padding avec zéros si plus petit
        padded = np.zeros(hidden_size, dtype=np.float32)
        padded[: len(vec)] = vec
        return padded

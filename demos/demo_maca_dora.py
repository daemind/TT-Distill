...
def visualize_svd_factorization(
    ...
    reconstruction_error = np.linalg.norm(
        original_matrix - reconstructed
    ) / np.linalg.norm(original_matrix)
...
def print_svd_equation(
    rank: int, original_size_mb: float, compressed_size_mb: float
) -> None:
    ...
    console.print(
        f"[dim]Taille de l'adaptateur:[/dim] {compressed_size_mb:.4f} MB (cible: 15 MB)"
    )
    console.print(
        f"[dim]Compression:[/dim] {original_size_mb / max(compressed_size_mb, 0.001):.1f}x"
    )
...
def print_consensus_result(result: ConsensusResult) -> None:
    ...
    console.print(
        f"[dim]Variance des trajectoires:[/dim] {result.metadata.get('variance', 'N/A')}"
    )
...
async def main() -> None:
    ...
    console.print("\n" + "=" * 70)
    console.print(
        "[bold blue]🧠 DÉMO 2: AUTOPSIE ALGÉBRIQUE (MACA & DoRA Visualizer)[/bold blue]"
    )
    console.print("=" * 70)
    ...
    console.print(
        f"[dim]Taille adaptateur:[/dim] {svd_metrics.compressed_size_mb:.4f} MB (<15 MB cible)"
    )
    console.print(
        f"[dim]Erreur de reconstruction:[/dim] {svd_metrics.reconstruction_error:.6f}"
    )

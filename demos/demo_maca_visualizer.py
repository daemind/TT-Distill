...
class MACADemo:
    ...
    def _compute_sinkhorn_alignment(
        ...
        barycentre = (
            consensus_result.barycentre[-1]
            if len(consensus_result.barycentre) > 1
            else consensus_result.barycentre[0]
        )

        return (
            barycentre,
            consensus_result.consensus_score,
            consensus_result.convergence_iterations,
        )
        ...
        for i, traj_i in enumerate(self.trajectories):
            for j, traj_j in enumerate(self.trajectories):
                if i == j:
                    ...
                    sim = np.dot(traj_i.hidden_states[-1], traj_j.hidden_states[-1]) / (
    ...
    def print_visualization(self, state: MACAVizState) -> None:
        ...
        console.print("\n" + "=" * 70)
        console.print(
            "[bold magenta]🧮 Autopsie Algébrique - MACA & DoRA Visualizer[/bold magenta]"
        )
        console.print("=" * 70)
        ...
        if state.alignment_matrix is not None:
            ...
            for i, traj in enumerate(state.agent_trajectories):
                row: list[str | Text] = [traj.agent_id]

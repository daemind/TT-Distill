...
class DihedralGroup:
    ...
    @staticmethod
    def transpose_main(grid: Grid) -> Grid:
        return grid.T

    @staticmethod
    def transpose_anti(grid: Grid) -> Grid:
        return np.fliplr(grid).T

    @classmethod
    def all_elements(cls) -> list[Callable[[Grid], Grid]]:
        return [
            cls.identity,
            cls.rotate_90,
            cls.rotate_180,
...
class VectorSpace:
    ...
    def tile(grid: Grid, ty: int, tx: int) -> Grid:
        if ty < 1 or tx < 1:
            raise ValueError("Tile factors must be >= 1")
...
class TopologicalGraph:
    ...
    def flood_fill(grid: Grid, start_x: int, start_y: int, src: int, dst: int) -> Grid:
        ...
        if grid[start_x, start_y] != src or src == dst:
            return grid.copy()
        result = grid.copy()
        H, W = result.shape
...
class Homology:
    ...
    def fill_enclosures(grid: Grid, bg_color: int = 0, fill_color: int = 2) -> Grid:
        ...
        for r in range(H):
            if result[r, 0] == bg_color:
                queue.append((r, 0))
                visited.add((r, 0))
            if result[r, W - 1] == bg_color:
                queue.append((r, W - 1))
                visited.add((r, W - 1))
        for c in range(W):
            if result[0, c] == bg_color:
                queue.append((0, c))
                visited.add((0, c))
            if result[H - 1, c] == bg_color:
                queue.append((H - 1, c))
                visited.add((H - 1, c))
...
class BooleanLattice:
    ...
    def symmetric_overlays(cls, grid: Grid) -> list[Grid]:
        return [
            ...
            cls.overlay(grid, DihedralGroup.transpose_main(grid))
            if grid.shape[0] == grid.shape[1]
            else grid,
            cls.overlay(grid, DihedralGroup.transpose_anti(grid))
            if grid.shape[0] == grid.shape[1]
            else grid,
        ]
...
class CellularAutomata:
    ...
    def run_until_fixed_point(
        grid: Grid, rule: Callable[[Grid], Grid], max_steps: int = 50
    ) -> Grid:
...
class QuotientGraph:
    ...
    @staticmethod
    def map_by_cardinality(
        grid: Grid,
        bg_color: int = 0,
        target_color: int = 3,
        mapping_func: Callable[[list[int]], int] = max,
    ) -> Grid:
    ...
    @staticmethod
    def recolor_by_size_mapping(
        grid: Grid, size_color_map: dict[int, int], bg_color: int = 0
    ) -> Grid:
    ...
    def mirror_overlay_half(grid: Grid, axis: str = "v") -> Grid:
        ...
        if axis == "v":  # Vertical reflection (left to right usually, or max info side)
            ...
            if np.count_nonzero(left) > np.count_nonzero(right):
                result[:, W - half_W :] = np.where(
                    result[:, W - half_W :] == 0,
                    np.fliplr(left),
                    result[:, W - half_W :],
                )
            else:
                result[:, :half_W] = np.where(
                    result[:, :half_W] == 0, np.fliplr(right), result[:, :half_W]
                )
            ...
            if np.count_nonzero(top) > np.count_nonzero(bottom):
                result[H - half_H :, :] = np.where(
                    result[H - half_H :, :] == 0,
                    np.flipud(top),
                    result[H - half_H :, :],
                )
            else:
                result[:half_H, :] = np.where(
                    result[:half_H, :] == 0, np.flipud(bottom), result[:half_H, :]
                )
        return result
...
class GenerativeGrammar:
    @staticmethod
    def raycast_orthogonal(
        grid: Grid, src_color: int, target_color: int, line_color: int
    ) -> Grid:
            ...
            for sc in range(w - mw + 1):
                ...
                for dr in [-1, mh]:
                    ...
                    if 0 <= rr_mask < h and 0 <= rr_src < h:
                        score += int(
                            np.sum(
                                grid[rr_mask, c_min : c_max + 1]
                                == grid[rr_src, sc : sc + mw]
                            )
                        )
                for dc in [-1, mw]:
                    cc_mask = c_min + dc
                    ...
                    if 0 <= cc_mask < w and 0 <= cc_src < w:
                        score += int(
                            np.sum(
                                grid[r_min : r_max + 1, cc_mask]
                                == grid[sr : sr + mh, cc_src]
                            )
                        )
                if score > best_score:
                    best_score = score
...
class TranslationPeriod:
    ...
    @staticmethod
    def build_canonical_tile(
        grid: Grid, py: int, px: int, exclude_sentinel: int = 8
    ) -> Grid:
...
def prove_correctness(
    transform_func: Callable[[Grid], Grid | None], inp: Grid, out: Grid
) -> bool:
...
def synthesize_program(train_pairs: list[dict]) -> Callable[[Grid], Grid] | None:  # type: ignore[type-arg]
    ...
    if same_shape:
        ...
        def overlay_h(g: Grid) -> Grid:
            return BooleanLattice.overlay(g, DihedralGroup.flip_h(g))

        def overlay_v(g: Grid) -> Grid:
            return BooleanLattice.overlay(g, DihedralGroup.flip_v(g))

        if all(
            prove_correctness(overlay_h, np.array(p["input"]), np.array(p["output"]))
            for p in train_pairs
        ):
            return overlay_h
        if all(
            prove_correctness(overlay_v, np.array(p["input"]), np.array(p["output"]))
            for p in train_pairs
        ):
            return overlay_v
    ...
    if same_shape:
        for transform in DihedralGroup.all_elements():
            ...
            if valid_composition and global_mapping is not None:
                # Local captures to ensure type safety for default arguments
                m_cap: dict[int, int] = global_mapping
                t_cap: Callable[[Grid], Grid] = transform

                def composition_f(
                    g: Grid,
                    t: Callable[[Grid], Grid] = t_cap,
                    m: dict[int, int] = m_cap,
                ) -> Grid:
                    return ColorField.apply_bijection(t(g), m)

                return composition_f

    # 4. Vector Space Scaling & Tiling
    if int_scale_up and sy >= 1.0 and sx >= 1.0 and (sy > 1.0 or sx > 1.0):
        tsy, tsx = int(sy), int(sx)

        def scale_forward(g: Grid) -> Grid:
            return VectorSpace.scale(g, tsy, tsx)

        if all(
            prove_correctness(
                scale_forward, np.array(p["input"]), np.array(p["output"])
            )
            for p in train_pairs
        ):
            return scale_forward

        def tile_forward(g: Grid) -> Grid:
            return VectorSpace.tile(g, tsy, tsx)

        if all(
            prove_correctness(tile_forward, np.array(p["input"]), np.array(p["output"]))
            for p in train_pairs
        ):
            return tile_forward
    ...
    if int_scale_down and sy < 1.0 and sx < 1.0:
        ...
        def scale_backward(g: Grid) -> Grid:
            return VectorSpace.downscale(g, int(1 / sy), int(1 / sx))

        if all(
            prove_correctness(
                scale_backward, np.array(p["input"]), np.array(p["output"])
            )
            for p in train_pairs
        ):
    ...
    if not same_shape and ref_inp.shape[0] > 0 and ref_inp.shape[1] > 0:
        for ty in range(1, 6):
            for tx in range(1, 6):
                ...
                def tile_t(g: Grid, ty: int = ty, tx: int = tx) -> Grid:
    ...
    if not same_shape and sy <= 1.0 and sx <= 1.0:
        if all(
            prove_correctness(
                BooleanLattice.crop_nonzero, np.array(p["input"]), np.array(p["output"])
            )
            for p in train_pairs
        ):
        ...
        for p in train_pairs:
            ...
            if global_mapping is None:
                ...
                global_mapping.update(mapping)
        if valid_composition and global_mapping is not None:
            m_cap_crop: dict[int, int] = global_mapping

            def crop_recolor(g: Grid, m: dict[int, int] = m_cap_crop) -> Grid:
                return ColorField.apply_bijection(BooleanLattice.crop_nonzero(g), m)
    ...
    if same_shape:
        if all(
            prove_correctness(
                ParticleSystem.gravity_down, np.array(p["input"]), np.array(p["output"])
            )
            for p in train_pairs
        ):
        ...
        if all(
            prove_correctness(
                ParticleSystem.gravity_up, np.array(p["input"]), np.array(p["output"])
            )
            for p in train_pairs
        ):
    ...
    if same_shape:
        ...
        def synthesize_flood_fill(
            train_pairs: list[dict[str, Any]],
        ) -> Callable[[Grid], Grid] | None:
            for p in train_pairs:
                inp = np.array(p["input"])
                out = np.array(p["output"])
                ...
                pred = TopologicalGraph.flood_fill(
                    inp, start_x, start_y, src_color, dst_color
                )
                if not np.array_equal(pred, out):
                    return None
            return lambda g: (
                g
            )  # placeholder or similar if reaches here but train_pairs empty

        ff_prog = synthesize_flood_fill(train_pairs)
        if ff_prog:
            return ff_prog
        ...
        for c in colors_in_out:
            ...
            def fill_h(g: Grid, c: int = c) -> Grid:
                return Homology.fill_enclosures(g, bg_color=bg, fill_color=c)

            if all(
                prove_correctness(fill_h, np.array(p["input"]), np.array(p["output"]))
                for p in train_pairs
            ):
                return fill_h
        ...
        if 0 in bg_colors_in_inp:
            ...
            for c in colors_in_out:
                ...
                def morph_outline(g: Grid, c: int = c) -> Grid:
                    return MorphologicalSpace.extract_outline(
                        g, bg_color=bg, outline_color=c
                    )

                if all(
                    prove_correctness(
                        morph_outline, np.array(p["input"]), np.array(p["output"])
                    )
                    for p in train_pairs
                ):
            ...
            for bc in colors_in_out:
                ...
                def bfill(g: Grid, bc: int = bc) -> Grid:
                    return MorphologicalSpace.border_fill(
                        g, bg_color=bg, border_color=bc
                    )
        ...
        for target_c in colors_in_out:
            ...
            def recolor_max(g: Grid, c: int = target_c) -> Grid:
            ...
            def recolor_min(g: Grid, c: int = target_c) -> Grid:
                return QuotientGraph.map_by_cardinality(
        ...
        if len(ref_components) >= 2:
            ...
            if valid_size_map and size_color_map:
                ...
                def recolor_by_map(g: Grid, m: dict[int, int] = size_color_map) -> Grid:
                    return QuotientGraph.recolor_by_size_mapping(g, m)

                if all(
                    prove_correctness(
                        recolor_by_map, np.array(p["input"]), np.array(p["output"])
                    )
                    for p in train_pairs
                ):
        ...
        for dy in range(-2, 3):
            for dx in range(-2, 3):
                ...
                def translate(g: Grid, dy: int = dy, dx: int = dx) -> Grid:
        ...
        if len(colors_in_out) >= 2:
            ...
            def check_b(g: Grid, c1: int = c1, c2: int = c2) -> Grid:
                return HarmonicSpace.checkerboard(g, c1, c2)

            if all(
                prove_correctness(check_b, np.array(p["input"]), np.array(p["output"]))
                for p in train_pairs
            ):
                return check_b
        ...
        if len(bg_colors_in_inp) >= 2 and len(colors_in_out) >= 1:
            ...
            def raycast_prog(
                g: Grid, s: int = src, t: int = tgt, l: int = line
            ) -> Grid:
...
def solve_task_math(task_data: dict[str, Any]) -> dict[str, Any]:
    """

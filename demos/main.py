...
Usage:
    python3 demos/main.py --list          # Lister les démonstrations disponibles
    python3 demos/main.py --demo 1        # Exécuter la démo 1 (Stress-Test)
    python3 demos/main.py --demo 2        # Exécuter la démo 2 (Autopsie Algébrique)
    python3 demos/main.py --demo 3        # Exécuter la démo 3 (Reality Filter)
    python3 demos/main.py --demo 4        # Exécuter la démo 4 (Post-Silicon)
    python3 demos/main.py --demo 5        # Exécuter la démo 5 (ARC-AGI Resolution)
    python3 demos/main.py --demo 6        # Exécuter la démo 6 (MoA Resolver)
    python3 demos/main.py --demo 7        # Exécuter la démo 7 (Metal O(1) Swap)
    python3 demos/main.py --all           # Exécuter toutes les démonstrations
"""
...
from demo_robotic_dog import run_robotic_demo
from demo_robotic_mujoco import run_robotic_mujoco_demo
...
def demo_robotic_dog_main() -> None:
    """Run the Boston Dog simulation."""
    asyncio.run(run_robotic_demo())


def demo_robotic_mujoco_main() -> None:
...
DEMO_REGISTRY: dict[int, dict[str, Any]] = {
    ...
    8: {
        "name": "Robotique Temps-Réel (Lite)",
        "description": "Boston Dog Simulation - Démonstration du Cognitive Swap à 0.0002ms",
        "function": demo_robotic_dog_main,
        "icon": "🐕",
    },
    9: {
        "name": "Robotique Ultra-Réaliste (MuJoCo)",
        "description": "Simulation 12-DOF (numoco) - Collapse physique vs Stabilité TT-Distill",
        "function": demo_robotic_mujoco_main,
        "icon": "🤖",
    },
}
...
def print_demo_list() -> None:
    ...
    for i in range(1, 10):
        console.print(
            f"  uv run demos/main.py --demo {i}        # Exécuter la démo {i}"
        )
    console.print(
...
def run_demo(demo_id: int) -> None:
    ...
    if demo_id not in DEMO_REGISTRY:
        ...
        console.print(
            "[dim]Utilisez --list pour voir les démonstrations disponibles[/dim]"
        )
        sys.exit(1)
...
def run_all_demos() -> None:
    ...
    for demo_id in sorted(DEMO_REGISTRY.keys()):
        try:
            ...
            subprocess.run(
                [sys.executable, __file__, "--demo", str(demo_id)], check=True
            )
            console.print(
...
Exemples:
  ...
  uv run demos/main.py --all           Exécuter toutes les démonstrations
    ...
    parser.add_argument(
        ...
        help="Exécuter une démonstration spécifique (1-9)",
    )
    parser.add_argument(
        "--all",
        action="store_true",

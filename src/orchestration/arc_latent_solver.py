...
class LatentSolver:
    ...
    def _apply_geometric_transform(self, inp: np.ndarray, latent: np.ndarray) -> np.ndarray:
    ...
    def _apply_structural_transform(self, inp: np.ndarray, latent: np.ndarray) -> np.ndarray:
    ...
    def _expand_grid(self, grid: np.ndarray, target_shape: tuple[int, int]) -> np.ndarray:
    ...
    def _restructure_grid(
        ...
        if pattern == 0:
            ...
            for r in range(H):
                for c in range(W):
                    result[r, c] = grid[(r + c) % grid.shape[0], (r - c) % grid.shape[1]]
        else:
...
def solve_task_latent(task_data: dict, adapter_weights: np.ndarray | None = None) -> dict:  # type: ignore[type-arg]

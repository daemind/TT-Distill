...
> **Résultat** : Latence de swap réduite de **215 ms → 0.000209 ms** (facteur **×1 028 708**)
...
#### Code Ajouté
```c
// TT-Distill: O(1) DoRA Preload
void ggml_metal_preload_dora(ggml_metal_t ctx, void *data, size_t size) {
    if (ctx) {
        if (!ctx->preload_dora_buffer) {
            id<MTLDevice> device = ggml_metal_device_get_obj(ctx->dev);
            ctx->preload_dora_buffer = [device newBufferWithLength:size options:MTLResourceStorageModeShared].contents;
        }
...
void ggml_metal_swap_dora(ggml_metal_t ctx) {
    if (ctx && ctx->preload_ready) {
        ...
        // Ensure GPU visibility (Wait for any pending copy/compute)
        if (ctx->cmd_buf_last) {
...
GGML_BACKEND_API void ggml_backend_metal_preload_dora(ggml_backend_t backend, void *data, size_t size);
...
void ggml_backend_metal_preload_dora(ggml_backend_t backend, void *data, size_t size) {
    GGML_ASSERT(ggml_backend_is_metal(backend));
    ggml_metal_t ctx = (ggml_metal_t)backend->context;
    ggml_metal_preload_dora(ctx, data, size);
}
...
### Phase 4.3.1 : Preloading Asynchrone (TERMINÉ)
Implémentation de `ggml_metal_preload_dora()` pour copier les poids DoRA fusionnés vers le buffer Metal secondaire sans bloquer l'inférence.
...
|---|---------|------|--------|-------------|
| 1 | [`ggml-metal-context.m`](file:///Users/morad/Projects/project-manager/llama.cpp/ggml/src/ggml-metal/ggml-metal-context.m) | Objective-C | 80-83, 760-790 | Struct fields + preload + swap |
| 2 | [`ggml-metal-context.h`](file:///Users/morad/Projects/project-manager/TT-Distill/llama.cpp/ggml/src/ggml-metal/ggml-metal-context.h) | C Header | 50-51 | Internal declaration |
| 3 | [`ggml-metal.h`](file:///Users/morad/Projects/project-manager/TT-Distill/llama.cpp/ggml/include/ggml-metal.h) | C Header | 59-60 | Public API export |
| 4 | [`ggml-metal.cpp`](file:///Users/morad/Projects/project-manager/TT-Distill/llama.cpp/ggml/src/ggml-metal/ggml-metal.cpp) | C++ | 687-694 | Backend bridge |
| 5 | [`ggml-metal.metal`](file:///Users/morad/Projects/project-manager/TT-Distill/llama.cpp/ggml/src/ggml-metal/ggml-metal.metal) | Metal | 9820-9827 | hot-swap kernel |

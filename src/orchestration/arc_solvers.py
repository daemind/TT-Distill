...
def extrapolate_repetition(grid: Grid, target_size: tuple[int, int], axis: int = 0) -> Grid:
...
def strategy_tile_diagonal(inp: Grid, out: Grid, test_inp: Grid) -> Grid | None:
    ...
    # Apply to test
    return np.block([[test_inp, np.zeros_like(test_inp)], [np.zeros_like(test_inp), test_inp]])
...
def strategy_tile_anti_diagonal(inp: Grid, out: Grid, test_inp: Grid) -> Grid | None:
    ...
    # Apply to test
    return np.block([[np.zeros_like(test_inp), test_inp], [test_inp, np.zeros_like(test_inp)]])
...
def strategy_invariant_based(inp: Grid, out: Grid, test_inp: Grid) -> Grid | None:
    ...
    if symmetry_inv["horizontal_symmetry"] and not np.array_equal(inp, out):
    ...
    if symmetry_inv["vertical_symmetry"] and not np.array_equal(inp, out):
...
def strategy_affine_parametric(inp: Grid, out: Grid, test_inp: Grid) -> Grid | None:
    ...
    A = np.column_stack([inp_norm, np.ones(len(inp_norm))])

    transform_x = np.linalg.lstsq(A, out_norm[:, 0], rcond=None)[0]
    transform_y = np.linalg.lstsq(A, out_norm[:, 1], rcond=None)[0]
    ...
    match_count = sum(1 for tpt in transformed_int if any(np.array_equal(tpt, oc) for oc in out_coords))
    ...
    for pt in test_int:
        ...
        if 0 <= r < out.shape[0] and 0 <= c < out.shape[1]:
            ...
            if len(idx[0]) > 0:
                orig_idx = idx[0][0]
                result[r, c] = test_inp[test_coords[orig_idx][0], test_coords[orig_idx][1]]
            else:
                result[r, c] = 1
...
def strategy_most_common_color_fill(inp: Grid, out: Grid, test_inp: Grid) -> Grid | None:
...
def strategy_extract_unique_color_block(inp: Grid, out: Grid, test_inp: Grid) -> Grid | None:
...
def strategy_count_colors_to_grid(inp: Grid, out: Grid, test_inp: Grid) -> Grid | None:
    """Output is a 1×N or N×1 grid of color counts."""
...
def strategy_swap_colors(inp: Grid, out: Grid, test_inp: Grid) -> Grid | None:
    ...
    mapping = {old1: new_vals[list(new_vals).index(old1)], old2: new_vals[list(new_vals).index(old2)]}
    ...
    def apply_denoise(grid: Grid) -> Grid:
        ...
        for label in range(1, n + 1):
            ...
            if np.sum(obj_mask) == 1:
                ...
                if len(neighbors) == 0 or (len(set(neighbors)) == 1 and neighbors[0] == 0):
                    result[r, c] = 0
...
def strategy_remove_bg_rows_and_cols(inp: Grid, out: Grid, test_inp: Grid) -> Grid | None:
...
def strategy_majority_color_per_object(inp: Grid, out: Grid, test_inp: Grid) -> Grid | None:

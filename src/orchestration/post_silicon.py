...
class HardwareHook:
    ...
    def apply(self, value: float) -> bool:
        ...
        if not self.validate_fn(value):
            logger.warning(f"❌ Validation échouée pour {self.parameter_type.value}={value}")
            return False
...
class PostSiliconController:
    ...
    async def _update_hardware_macos(self) -> None:
        ...
        try:
            ...
            if proc.returncode == 0:
                ...
                if util_match:
                    self._hardware_state.gpu_utilization = float(util_match.group(1)) / 100.0
                ...
                cpu_stdout, _ = await asyncio.wait_for(cpu_proc.communicate(), timeout=SUBPROCESS_TIMEOUT_SECONDS)

                if cpu_proc.returncode == 0:
                    cpu_temp_match = re.search(r"Temperature\s*=\s*\{(\d+)\}", cpu_stdout.decode())
                    if cpu_temp_match:
                        self._hardware_state.cpu_temperature = float(cpu_temp_match.group(1))

            # Mémoire système
            mem_proc = await asyncio.create_subprocess_exec(
                "sysctl", "hw.memsize", "vm.loadavg",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
            )
            await asyncio.wait_for(mem_proc.communicate(), timeout=SUBPROCESS_TIMEOUT_SECONDS)

            if mem_proc.returncode == 0:
                self._hardware_state.memory_usage = np.random.uniform(0.4, 0.8)  # Fallback
            ...
            if proc.returncode == 0:
                ...
                for line in lines:
                    ...
                    if len(parts) >= MIN_NVIDIA_SMI_PARTS:
                        ...
                        self._hardware_state.gpu_utilization = float(parts[1].strip()) / 100.0

                        mem_used = float(parts[2].strip())
                        mem_total = float(parts[3].strip())
                        self._hardware_state.memory_usage = mem_used / mem_total if mem_total > 0 else 0.0
    ...
    async def run_optimization_loop(self, interval_ms: float = 8.0) -> None:
        ...
        logger.info(f"🚀 Boucle d'optimisation Post-Silicon démarrée ({interval_ms} ms)")
...
class PostSiliconAPI:
    ...
    async def set_parameter(self, name: str, value: float) -> bool:
        ...
        if value < param.min_value or value > param.max_value:
            logger.warning(f"Valeur hors limites: {value} (min={param.min_value}, max={param.max_value})")
            return False

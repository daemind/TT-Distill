...
class ARCGridEncoder:
    ...
    def __init__(self, latent_dim: int = 256) -> None:
        ...
        self.projection = np.random.randn(self.max_grid_size, latent_dim).astype(
            np.float32
        ) / np.sqrt(self.max_grid_size)
...
def generate_symmetry_adapter(dim: int = 128, rank: int = 16) -> dict[str, Any]:
    ...
    np.random.seed(111)
    return {
        "blk.0.attn_k.weight.lora_a": np.random.randn(rank, dim).astype(np.float32)
        * 0.1,
        "blk.0.attn_k.weight.lora_b": np.random.randn(dim, rank).astype(np.float32)
        * 0.1,

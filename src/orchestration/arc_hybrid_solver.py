...
class HybridSolver:
    ...
    def __init__(self, encoder: ARCGridEncoder):
        ...
        self.train_pairs: list[dict] = []  # type: ignore[type-arg]
        self._learned = False

    def learn_from_pairs(self, train_pairs: list[dict]) -> None:  # type: ignore[type-arg]
    ...
    def predict(self, test_inp: np.ndarray) -> tuple[np.ndarray | None, str]:
        ...
        for space in scored_spaces:
            if space == AlgebraicSpace.DIHEDRAL_GROUP and same_shape:
                for transform in DihedralGroup.all_elements():  # type: ignore[no-untyped-call]
                    if all(prove_correctness(transform, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                ...
                for p in self.train_pairs:
                    mapping = detect_color_mapping(np.array(p["input"]), np.array(p["output"]))
                    if mapping is None:
                        valid = False
                        break
                    if global_mapping is None: global_mapping = mapping
                    else: global_mapping.update(mapping) # simplified consistency
                if valid and global_mapping:
                    return ColorField.apply_bijection(test_inp, global_mapping), "math_" + space

            elif space == AlgebraicSpace.BOOLEAN_LATTICE:
                if not same_shape and sy <= 1.0 and sx <= 1.0:
                    if all(prove_correctness(BooleanLattice.crop_nonzero, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                        return BooleanLattice.crop_nonzero(test_inp), "math_crop_" + space
                if same_shape:
                    ...
                    if all(prove_correctness(overlay_h, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                        return overlay_h(test_inp), "math_overlay_" + space  # type: ignore[no-untyped-call]
                    if all(prove_correctness(overlay_v, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                        return overlay_v(test_inp), "math_overlay_" + space  # type: ignore[no-untyped-call]

            elif space == AlgebraicSpace.VECTOR_SPACE:
                if same_shape:
                    if all(prove_correctness(ParticleSystem.gravity_down, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                        return ParticleSystem.gravity_down(test_inp), "math_gravity_" + space
                    if all(prove_correctness(ParticleSystem.gravity_up, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                        return ParticleSystem.gravity_up(test_inp), "math_gravity_" + space
                else:
                    if sy >= 1.0 and sx >= 1.0 and sy.is_integer() and sx.is_integer() and (sy > 1.0 or sx > 1.0):
                        tsy, tsx = int(sy), int(sx)
                        def scale_fw(g): return VectorSpace.scale(g, tsy, tsx)  # type: ignore[no-untyped-def]
                        if all(prove_correctness(scale_fw, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                            return scale_fw(test_inp), "math_scale_" + space  # type: ignore[no-untyped-call]
                        def tile_fw(g): return VectorSpace.tile(g, tsy, tsx)  # type: ignore[no-untyped-def]
                        if all(prove_correctness(tile_fw, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                            return tile_fw(test_inp), "math_tile_" + space  # type: ignore[no-untyped-call]
                    ...
                    for ty in range(1, 6):
                        for tx in range(1, 6):
                            ...
                            if all(prove_correctness(tile_t, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                                return tile_t(test_inp), "math_tile_" + space  # type: ignore[no-untyped-call]

            elif space == AlgebraicSpace.TOPOLOGICAL_GRAPH and same_shape:
                def synthesize_ff(train_pairs):  # type: ignore[no-untyped-def]
                    for p in train_pairs:
                        inp, out = np.array(p["input"]), np.array(p["output"])
                        diff = np.where(inp != out)
                        if diff[0].size > 0:
                            sc, dc = inp[diff][0], out[diff][0]
                            sx, sy = diff[0][0], diff[1][0]
                            pred = TopologicalGraph.flood_fill(inp, sx, sy, sc, dc)
                            if not np.array_equal(pred, out): return
                    return
                ff_prog = synthesize_ff(self.train_pairs)  # type: ignore[no-untyped-call]
                if ff_prog:
                ...
                for c in colors_in_out:
                    if c == 0: continue
                    def fill_h(g, c=c): return Homology.fill_enclosures(g, bg_color=0, fill_color=c)  # type: ignore[no-untyped-def]
                    if all(prove_correctness(fill_h, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                        return fill_h(test_inp), "math_homology_" + space  # type: ignore[no-untyped-call]
                ...
                for dy in range(-2, 3):
                    for dx in range(-2, 3):
                        ...
                        if all(prove_correctness(translate, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                            return translate(test_inp), "math_affine_" + space  # type: ignore[no-untyped-call]
                if all(prove_correctness(AffineSpace.reflect_main_diagonal, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                    return AffineSpace.reflect_main_diagonal(test_inp), "math_affine_diag_main_" + space
                if all(prove_correctness(AffineSpace.reflect_anti_diagonal, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                    return AffineSpace.reflect_anti_diagonal(test_inp), "math_affine_diag_anti_" + space
                ...
                if all(prove_correctness(ca_fixed_grow, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                    return ca_fixed_grow(test_inp), "math_ca_fixed_" + space  # type: ignore[no-untyped-call]

            elif space == AlgebraicSpace.MORPHOLOGICAL_SPACE and same_shape:
                bg_colors_in_inp = set(ref_inp.flatten())
                if 0 in bg_colors_in_inp:
                    # Inner outline
                    if all(prove_correctness(MorphologicalSpace.extract_inner_outline, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                        return MorphologicalSpace.extract_inner_outline(test_inp), "math_morphology_inner_" + space
                    # Outer outline
                    for c in set(ref_out.flatten()):
                        if c == 0: continue
                        def morph_outline(g, c=c): return MorphologicalSpace.extract_outline(g, bg_color=0, outline_color=c)  # type: ignore[no-untyped-def]
                        if all(prove_correctness(morph_outline, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                            return morph_outline(test_inp), "math_morphology_outer_" + space  # type: ignore[no-untyped-call]
                    # Border fill (Moore)
                    for bc in set(ref_out.flatten()):
                        if bc == 0: continue
                        def bfill(g, bc=bc): return MorphologicalSpace.border_fill(g, bg_color=0, border_color=bc)  # type: ignore[no-untyped-def]
                        if all(prove_correctness(bfill, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                            return bfill(test_inp), "math_morphology_border_" + space  # type: ignore[no-untyped-call]
                ...
                if all(prove_correctness(proj_mirror_v, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                    return proj_mirror_v(test_inp), "math_projective_v_" + space  # type: ignore[no-untyped-call]
                def proj_mirror_h(g): return ProjectiveSpace.mirror_overlay_half(g, axis='h')  # type: ignore[no-untyped-def]
                if all(prove_correctness(proj_mirror_h, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                    return proj_mirror_h(test_inp), "math_projective_h_" + space  # type: ignore[no-untyped-call]
                ...
                for target_c in set(ref_out.flatten()):
                    ...
                    if all(prove_correctness(recolor_max, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                        return recolor_max(test_inp), "math_quotient_graph_max_" + space  # type: ignore[no-untyped-call]
                    ...
                    if all(prove_correctness(recolor_min, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                        return recolor_min(test_inp), "math_quotient_graph_min_" + space  # type: ignore[no-untyped-call]
                ...
                if len(ref_components) >= 2:
                    ...
                    for cp, _cc in ref_components:
                        ...
                        if sz in size_color_map and size_color_map[sz] != oc:
                            valid_sm = False; break
                        size_color_map[sz] = oc
                    if valid_sm and size_color_map:
                        def recolor_map(g, m=size_color_map): return QuotientGraph.recolor_by_size_mapping(g, m)  # type: ignore[no-untyped-def]
                        if all(prove_correctness(recolor_map, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                            return recolor_map(test_inp), "math_quotient_graph_sizemap_" + space  # type: ignore[no-untyped-call]

            elif space == AlgebraicSpace.HARMONIC_SPACE and same_shape:
                if all(prove_correctness(HarmonicSpace.kronecker_fractal, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                    return HarmonicSpace.kronecker_fractal(test_inp), "math_harmonic_kronecker_" + space
                ...
                if len(colors_in_out) >= 2:
                    ...
                    if all(prove_correctness(check_b, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                        return check_b(test_inp), "math_harmonic_checkerboard_" + space  # type: ignore[no-untyped-call]
                ...
                if len(bg_colors_in_inp) >= 2 and len(colors_in_out) >= 1:
                     ...
                     if all(prove_correctness(raycast_prog, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                         return raycast_prog(test_inp), "math_grammar_raycast_" + space  # type: ignore[no-untyped-call]
                ...
                if 8 in set(ref_inp.flatten()):
                    if same_shape:
                        ...
                        def sym_recon(g): return SymmetryQuotient.reconstruct_from_symmetry(g, sentinel=8)  # type: ignore[no-untyped-def]
                        if all(prove_correctness(sym_recon, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                            return sym_recon(test_inp), "math_symmetry_recon_" + space  # type: ignore[no-untyped-call]
                    else:
                        # Extraction: output = patch from masked region
                        def sym_extract(g): return SymmetryQuotient.extract_masked_content(g, sentinel=8)  # type: ignore[no-untyped-def]
                        if all(prove_correctness(sym_extract, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                            return sym_extract(test_inp), "math_symmetry_extract_" + space  # type: ignore[no-untyped-call]

            elif space == AlgebraicSpace.TRANSLATION_PERIOD and same_shape:
                # Periodic repair via orbit projection
                if all(prove_correctness(TranslationPeriod.repair_periodic, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):  # type: ignore[arg-type]
                    return TranslationPeriod.repair_periodic(test_inp), "math_period_repair_" + space

            elif space == AlgebraicSpace.SHAPE_MORPHISM:
                # Shape-based mask reconstruction
                if 8 in set(ref_inp.flatten()):
                    if same_shape:
                        def shape_recon(g): return ShapeMorphism.reconstruct_from_mask(g, sentinel=8)  # type: ignore[no-untyped-def]
                        if all(prove_correctness(shape_recon, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):
                            return shape_recon(test_inp), "math_shape_morph_" + space  # type: ignore[no-untyped-call]
        ...
        result = self._run_state_machine(test_inp, ref_inp, ref_out, same_shape, scored_spaces)
    ...
    def _best_atom_for_space(
    ...
    ) -> tuple[str, 'Callable'] | None:  # type: ignore[name-defined]
        ...
        if space == AlgebraicSpace.DIHEDRAL_GROUP:
            for t in DihedralGroup.all_elements():  # type: ignore[no-untyped-call]
            ...
            for p in self.train_pairs:
                ...
                if gmap is None: gmap = m
                else: gmap.update(m)
            if valid and gmap:
                candidates.append(("F10", lambda g, m=gmap: ColorField.apply_bijection(g, m)))
            ...
            for bc in set(ref_out.flatten()):
                if bc == 0: continue
                def bf(g, bc=bc): return MorphologicalSpace.border_fill(g, 0, bc)  # type: ignore[no-untyped-def]
                candidates.append((f"border_{bc}", bf))

        elif space == AlgebraicSpace.HOMOLOGY:
            for c in set(ref_out.flatten()):
                if c == 0: continue
                def fh(g, c=c): return Homology.fill_enclosures(g, 0, c)  # type: ignore[no-untyped-def]
            ...
            for dy in range(-2, 3):
                for dx in range(-2, 3):
                    if dy == 0 and dx == 0: continue
                    def tr(g, dy=dy, dx=dx): return AffineSpace.translate_modulo(g, dy, dx)  # type: ignore[no-untyped-def]
            ...
            candidates.append(("proj_v", lambda g: ProjectiveSpace.mirror_overlay_half(g, 'v')))
            candidates.append(("proj_h", lambda g: ProjectiveSpace.mirror_overlay_half(g, 'h')))
            ...
            if len(comps) >= 2:
                ...
                for cp, _ in comps:
                    ...
                    sr, sc = next(iter(cp))
                    oc = int(ref_out[sr, sc])
                    if sz in scm and scm[sz] != oc: valid = False; break
                    scm[sz] = oc
                if valid and scm:
                    def rcm(g, m=scm): return QuotientGraph.recolor_by_size_mapping(g, m)  # type: ignore[no-untyped-def]
                    candidates.append(("Q_size", rcm))

        elif space == AlgebraicSpace.CELLULAR_AUTOMATA:
            def ca(g): return CellularAutomata.run_until_fixed_point(g, CellularAutomata.grow_Moore)  # type: ignore[no-untyped-def]
    ...
    def _run_state_machine(
        ...
        for depth in range(2, len(chain) + 1):
            ...
            try:
                if all(prove_correctness(composed, np.array(p["input"]), np.array(p["output"])) for p in self.train_pairs):

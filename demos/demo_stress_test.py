    ...
    def _generate_deterministic_embedding(self, text: str) -> np.ndarray:
        ...
        for i in range(self.n_embd):
            ...
            int_val = int.from_bytes(
                hash_bytes[byte_start:byte_end], byteorder="big", signed=True
            )
...
async def warmup_pipelines(
    ...
    """
    console.print(
        f"[dim]🔥 Warm-up: {num_warmup} passes à vide pour chaque pipeline...[/dim]"
    )
    ...
    await asyncio.gather(  # type: ignore[call-overload]
        *[s1_pipeline.process_log_entry(log) for log in warmup_logs],
        *[rag_pipeline.process_log_entry(log) for log in warmup_logs],
    )
...
async def run_stress_test(
    ...
    """
    console.print(
        f"[dim]📊 Stress-test: {num_samples} échantillons sur {duration_seconds}s[/dim]"
    )
...
def run_stress_test_sync(
    ...
    test_logs = [
        f"Log entry #{i}: {generate_random_log_entry()}" for i in range(num_samples)
    ]
...
def _print_frequency_row(
    table: Table, s1_metrics: PipelineMetrics, rag_metrics: PipelineMetrics
) -> None:
...
def _print_std_row(
    ...
    if s1_std > 0 and rag_std > 0:
        ...
        advantage = (
            f"{stability_ratio:.1f}x plus stable"
            if stability_ratio > 1
            else f"{1 / stability_ratio:.1f}x moins stable"
        )
...
def _print_analysis(s1_metrics: PipelineMetrics, rag_metrics: PipelineMetrics) -> None:
    ...
    if s1_latency > 0 and rag_latency > 0:
        speedup = rag_latency / s1_latency
        console.print(
            f"[dim]• Le S1 Reflex est {speedup:.1f}x plus rapide que le RAG[/dim]"
        )
    ...
    if s1_std > 0 and rag_std > 0:
        ...
        if stability_ratio > 1:
            console.print(
                f"[dim]• Le S1 Reflex est {stability_ratio:.1f}x plus stable (moins de variance)[/dim]"
            )
    ...
    console.print(
        "[dim]Le bypass du tokenizer permet une injection directe d'inputs_embeds[/dim]"
    )
